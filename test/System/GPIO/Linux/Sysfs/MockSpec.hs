    ...
    describe "SysfsMockT" $ do
      ...
      context "getDirectoryContents" $ do
        ...
        it "returns failure on files" $ do
          evalSysfsMock (getDirectoryContents "/sys/class/gpio/export") initialMockWorld [] `shouldBe` Left (NotADirectory "/sys/class/gpio/export")
        it "returns failure on non-existent names" $ do
          evalSysfsMock (getDirectoryContents "/sys/class/foobar") initialMockWorld [] `shouldBe` Left (NoSuchFileOrDirectory "/sys/class/foobar")

      context "readFile" $ do
        it "fails on /sys/class/gpio/export" $
          evalSysfsMock (readFile "/sys/class/gpio/export") initialMockWorld [] `shouldBe` Left (WriteOnlyFile "/sys/class/gpio/export")
        it "fails on /sys/class/gpio/unexport" $
          evalSysfsMock (readFile "/sys/class/gpio/unexport") initialMockWorld [] `shouldBe` Left (WriteOnlyFile "/sys/class/gpio/unexport")
        it "fails on non-existent file" $
          evalSysfsMock (readFile "/sys/class/gpio/foo") initialMockWorld [] `shouldBe` Left (NotAFile "/sys/class/gpio/foo")
        it "fails on a directory" $
          evalSysfsMock (readFile "/sys/class/gpio") initialMockWorld [] `shouldBe` Left (NotAFile "/sys/class/gpio")
      ...
      context "runSysfsMockT" $ do
        ...
        it "fails when MockGpioChips overlap" $ do
          evalSysfsMock (readFile "/sys/class/gpio/gpiochip16/ngpio") initialMockWorld [chip0, chip16, invalidChip32] `shouldBe` Left (PinAlreadyExists $ Pin 47)
